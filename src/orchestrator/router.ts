/**
 * Routing Guard — in-process (ported from PR #2 packages/orchestration-service/src/router.ts)
 *
 * Deterministic routing table derived from knowledge/05_agent_registry.md.
 *
 * Maps a TriageResult to exactly one of the three downstream in-process services:
 *   - 'knowledge'   → knowledge agent (src/agents/knowledge.ts)
 *   - 'issue'       → issue agent     (src/agents/issue.ts)
 *   - 'ticket'      → ticket agent    (src/agents/ticket.ts)
 *
 * Routing table (identical to PR #2):
 *   SOFTWARE         → issue   (Engineering Agent path)
 *   SECURITY         → issue   (Incident Agent path)
 ...
 *   HARDWARE         → ticket
 *   ACCESS           → ticket
 *   UNKNOWN          → knowledge  (best-effort; escalates to human on empty result)
 *
 * CRITICAL override:
 ...
 *
 * Signature kept stable for future extraction to a dedicated service.
 */
...
export function getRoute(triageResult: TriageResult): AgentServiceRoute {
  ...
  switch (triageResult.domain) {
    ...
    case 'UNKNOWN':
    default:
      // Best-effort knowledge lookup; machine escalates to human if
